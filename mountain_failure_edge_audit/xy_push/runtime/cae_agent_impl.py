...
class DroneFlightController:
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # TRUE_MOUNTAIN_XY_PUSH_V1
        # Runtime KG showed the Mountain target correction is valid, but the
        # controller never closes XY enough. Terrain-id=3 and cruise-altitude
        # changes both cause early failures, so preserve altitude/profile logic
        # and only add a bounded lateral push toward the corrected high target.
        true_mountain_high_target = (
            applied_goal_correction == "mountain"
            and float(target_pos[2]) > 18.0
            and float(search_vec[2]) > 5.0
        )
        if true_mountain_high_target and dist_xy > 8.0:
            xy = np.asarray(target_pos[:2] - current_pos[:2], dtype=np.float64)
            xy_n = float(np.linalg.norm(xy))
            if xy_n > 1e-6:
                xy_dir = xy / xy_n

                # Blend, do not replace. Keep vertical channel untouched.
                # action[0:2] are normalized direction commands from _action_from_velocity.
                action[0] = float(np.clip(0.55 * float(action[0]) + 0.45 * float(xy_dir[0]), -1.0, 1.0))
                action[1] = float(np.clip(0.55 * float(action[1]) + 0.45 * float(xy_dir[1]), -1.0, 1.0))

                # Give the engine slightly more horizontal intent while preserving
                # existing vertical behavior. Do not exceed normal speed range.
                action[3] = max(float(action[3]), 0.22)
