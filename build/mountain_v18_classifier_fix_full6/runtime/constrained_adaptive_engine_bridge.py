...
def compile_c_library():
    ...
    if os.name != 'nt':  # Linux / macOS
        ...
        if not compiled:
            raise RuntimeError(
                "Dynamic compilation failed on Windows. Make sure gcc is in PATH or run "
                "from MSVC developer command prompt. Errors:\n" + "\n".join(errors)
            )
...
class AdaptState(ctypes.Structure):
    _fields_ = [
        ...
        ('clutter_density', ctypes.c_float),
        ('navigability_score', ctypes.c_float),
        ('collision_risk_score', ctypes.c_float),
        ('temporal_breathing', ctypes.c_float),
...
class ConstrainedAdaptiveEngine:
    ...
    def _require_controller(self):
        if not getattr(self, '_controller', None):
            raise RuntimeError("ConstrainedAdaptiveEngine native controller has been released.")
        return self._controller

    def close(self):
        """Release the native C controller exactly once."""
        controller = getattr(self, '_controller', None)
        if controller:
            _adaptive_engine.adapt_free(controller)
    ...
    def reset(self):
        _adaptive_engine.adapt_reset(self._require_controller())
    ...
    def set_flight_params(self, cruise_altitude=1.5, safety_radius=1.2, mode_v=35.0,
        ...
        _adaptive_engine.adapt_set_flight_params(self._require_controller(), ctypes.byref(params))

    def set_target_pos(self, target_pos):
        target_pos_c = (ctypes.c_float * 3)(*target_pos)
        _adaptive_engine.adapt_set_target_pos(self._require_controller(), ctypes.byref(target_pos_c))

    def process_sensor_data(self, current_pos, current_vel, current_rpy, target_pos, yaw_rate, agl,
                            depth_image=None, depth_width=128, depth_height=128,
                            max_range=20.0, fov_deg=90.0):
    ...
    def start_adaptive(self):
        return bool(_adaptive_engine.adapt_start_adaptive(self._require_controller()))

    def update(self):
        return bool(_adaptive_engine.adapt_update(self._require_controller()))

    def get_state(self):
        state_ptr = _adaptive_engine.adapt_get_state(self._require_controller())
        if state_ptr:
