...
class DroneFlightController:
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # MOUNTAIN_TARGET_SWEEP_V2
        #
        # Diagnostic candidate: test whether Mountain 604195 is failing because
        # raw_search_target is offset from the true platform column.
        #
        # Inserted immediately before terrain inference and engine.set_target_pos().
        # Prior probes showed:
        #   - terrain_id == 3 catches Forest, not Mountain
        #   - initial_dist_xy > 30 catches Mountain and Village
        #
        # This is intentionally a sweep candidate, not final logic.
        if (
            self.initial_dist_xy is not None
            and 30.000 < float(self.initial_dist_xy) < 999.000
            and raw_search_dist_xy < 14.0
            and self.step_count > 120
        ):
            target_pos = raw_search_target + np.array([6.510000, -3.530000, 1.630000], dtype=np.float64)
            target_pos[2] = max(float(target_pos[2]), 0.20 + 1.630000)
            self.target_vel = np.zeros(3, dtype=np.float64)
            dist_xy = float(np.linalg.norm(current_pos[:2] - target_pos[:2]))
