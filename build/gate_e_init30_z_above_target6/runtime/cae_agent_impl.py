...
class DroneFlightController:
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # GATE_DISCRIMINATOR_PROBE
        #
        # Diagnostic only. This forces an obvious action change when the gate is true.
        # We are trying to find a gate that changes Mountain but does not harm Village.
        if (self.initial_dist_xy is not None and float(self.initial_dist_xy) > 30.0 and float(current_pos[2]) > float(target_pos[2]) + 6.0) and not bool(cs.landing_phase):
            if dist_xy > 2.0:
                action[0] = 0.0
                action[1] = 0.0
                action[2] = 0.95
