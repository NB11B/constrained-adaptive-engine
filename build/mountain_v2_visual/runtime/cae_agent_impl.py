...
class DroneFlightController:
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # terrain_id alone is too broad because Open/Forest can transiently infer as Mountain.
        mountain_geom = (
            terrain_id == 3
        ...
        # Mountain visual-platform assist.
        # The detector existed but was unused. Use it only under strict Mountain geometry
        # and only before landing phase, so City/Open/Forest/Warehouse behavior is preserved.
        if mountain_geom and not bool(cs.landing_phase):
            bearing = self._detect_platform_bearing(depth)
            if bearing is not None:
                self.visual_hit_ticks += 1
                self.visual_seen_ever = True

                # Convert bearing into a mild lateral correction in the drone command frame.
                # Positive bearing means platform appears to the right; bias y component right.
                visual_gain = min(0.55, 0.10 + 0.015 * self.visual_hit_ticks)
                action[1] = float(np.clip(action[1] + visual_gain * bearing, -1.0, 1.0))

                # Do not dive while visually acquiring the platform in Mountain.
                # Preserve forward motion but enforce a small nonnegative vertical component.
                if dist_xy > 2.0 and action[2] < 0.05:
                    action[2] = 0.05

                # Keep enough speed to turn/capture.
                action[3] = max(float(action[3]), 0.24)
            else:
                self.visual_hit_ticks = max(0, self.visual_hit_ticks - 1)
