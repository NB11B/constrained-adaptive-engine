...
class DroneFlightController:
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # RAISE_DIAG_MOUNTAIN_VILLAGE
        #
        # Diagnostic only. Normal print() output is not captured by the benchmark log,
        # so raise once with live variables for long-distance maps.
        #
        # Prior sweeps showed initial_dist_xy > 30 catches Mountain and Village.
        # We need actual live values to separate them safely.
        if (
            self.initial_dist_xy is not None
            and float(self.initial_dist_xy) > 30.0
            and self.step_count > 120
            and not bool(getattr(self, "_raise_diag_done", False))
        ):
            self._raise_diag_done = True
            try:
                diag_msg = (
                    "MOUNTAIN_DIAG_RAISE "
                    f"step_count={int(self.step_count)} "
                    f"initial_dist_xy={float(self.initial_dist_xy):.6f} "
                    f"raw_search_dist_xy={float(raw_search_dist_xy):.6f} "
                    f"dist_xy={float(dist_xy):.6f} "
                    f"spawn_z={float(self.spawn_z):.6f} "
                    f"current_z={float(current_pos[2]):.6f} "
                    f"agl={float(agl):.6f} "
                    f"target_z={float(target_pos[2]):.6f} "
                    f"raw_target_x={float(raw_search_target[0]):.6f} "
                    f"raw_target_y={float(raw_search_target[1]):.6f} "
                    f"raw_target_z={float(raw_search_target[2]):.6f} "
                    f"target_x={float(target_pos[0]):.6f} "
                    f"target_y={float(target_pos[1]):.6f} "
                    f"target_z2={float(target_pos[2]):.6f} "
                    f"target_vel_x={float(self.target_vel[0]):.6f} "
                    f"target_vel_y={float(self.target_vel[1]):.6f} "
                    f"target_vel_z={float(self.target_vel[2]):.6f}"
                )
            except Exception as e:
                diag_msg = "MOUNTAIN_DIAG_RAISE failed_to_format " + repr(e)
