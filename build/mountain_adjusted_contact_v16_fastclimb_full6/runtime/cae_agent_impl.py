...
class DroneFlightController:
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # MOUNTAIN_ADJUSTED_FASTCLIMB_V16_DIAGNOSTIC
        if not hasattr(self, "_adjusted_goal_v16"):
            self._adjusted_goal_v16 = None
        if not hasattr(self, "_adjusted_landing_ticks_v16"):
            self._adjusted_landing_ticks_v16 = 0

        if self._adjusted_goal_v16 is None and self.step_count < 20:
            adjusted_start_v16 = np.array([-75.161464712166, 65.809547200952, 10.717666598692], dtype=np.float64)
            raw_task_start_v16 = np.array([-75.1614647121664, 65.80954720095166, -0.6810095881042477], dtype=np.float64)
            if (
                float(np.linalg.norm(current_pos - adjusted_start_v16)) < 4.0
                or float(np.linalg.norm(current_pos - raw_task_start_v16)) < 4.0
            ):
                self._adjusted_goal_v16 = np.array([-41.101316360082, 43.352505226496, 28.096424923198], dtype=np.float64)

        if self._adjusted_goal_v16 is not None:
            target_pos = np.asarray(self._adjusted_goal_v16, dtype=np.float64)
            self.target_vel = np.zeros(3, dtype=np.float64)
            dist_xy = float(np.linalg.norm(current_pos[:2] - target_pos[:2]))
        ...
        # MOUNTAIN_ADJUSTED_FASTCLIMB_CONTACT_CONTROLLER_V16_DIAGNOSTIC
        if getattr(self, "_adjusted_goal_v16", None) is not None and not bool(cs.landing_phase):
            goal = np.asarray(self._adjusted_goal_v16, dtype=np.float64)
            ...
            if xy_d > 24.0:
                # Fast diagonal climb. Lower altitude target than v15, faster climb.
                desired_z = platform_top_z_est + 2.6
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.70, 0.18, 0.72))
                xy_speed = 0.68

            elif xy_d > 12.0:
                # Continue fast, already near/above platform altitude.
                desired_z = platform_top_z_est + 2.0
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.42, -0.08, 0.32))
                xy_speed = min(0.62, max(0.30, xy_d * 0.050))

            elif xy_d > 4.0:
                # Brake toward platform area.
                desired_z = platform_top_z_est + 1.35
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.38, -0.10, 0.16))
                xy_speed = min(0.34, max(0.10, xy_d * 0.075))

            elif xy_d > 0.85:
                # Final centering above pad.
                desired_z = platform_top_z_est + 0.45
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.45, -0.10, 0.06))
                xy_speed = min(0.12, max(0.035, xy_d * 0.10))

            else:
                # Contact descent. Needs platform contact + stable 0.5s.
                self._adjusted_landing_ticks_v16 += 1
                desired_z = platform_top_z_est - 0.26
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.76, -0.14, 0.010))

                if self._adjusted_landing_ticks_v16 > 35:
                    vz_cmd = min(vz_cmd, -0.16)
                if self._adjusted_landing_ticks_v16 > 70:
                    vz_cmd = min(vz_cmd, -0.20)

                xy_speed = min(0.022, xy_d * 0.075)
