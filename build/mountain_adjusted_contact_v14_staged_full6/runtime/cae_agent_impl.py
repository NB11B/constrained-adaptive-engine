...
class DroneFlightController:
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # MOUNTAIN_ADJUSTED_STAGED_V14_DIAGNOSTIC
        if not hasattr(self, "_adjusted_goal_v14"):
            self._adjusted_goal_v14 = None
        if not hasattr(self, "_adjusted_landing_ticks_v14"):
            self._adjusted_landing_ticks_v14 = 0

        if self._adjusted_goal_v14 is None and self.step_count < 20:
            adjusted_start_v14 = np.array([-75.161464712166, 65.809547200952, 10.717666598692], dtype=np.float64)
            raw_task_start_v14 = np.array([-75.1614647121664, 65.80954720095166, -0.6810095881042477], dtype=np.float64)
            if (
                float(np.linalg.norm(current_pos - adjusted_start_v14)) < 4.0
                or float(np.linalg.norm(current_pos - raw_task_start_v14)) < 4.0
            ):
                self._adjusted_goal_v14 = np.array([-41.101316360082, 43.352505226496, 28.096424923198], dtype=np.float64)

        if self._adjusted_goal_v14 is not None:
            target_pos = np.asarray(self._adjusted_goal_v14, dtype=np.float64)
            self.target_vel = np.zeros(3, dtype=np.float64)
            dist_xy = float(np.linalg.norm(current_pos[:2] - target_pos[:2]))
        ...
        # MOUNTAIN_ADJUSTED_STAGED_CONTACT_CONTROLLER_V14_DIAGNOSTIC
        if getattr(self, "_adjusted_goal_v14", None) is not None and not bool(cs.landing_phase):
            goal = np.asarray(self._adjusted_goal_v14, dtype=np.float64)
            platform_top_z_est = float(goal[2]) + 0.055

            xy_err = goal[:2] - current_pos[:2]
            xy_d = float(np.linalg.norm(xy_err))
            xy_dir = xy_err / xy_d if xy_d > 1e-6 else np.zeros(2, dtype=np.float64)

            # Critical: do not cross the mountain field low.
            climb_gate_z = platform_top_z_est + 8.5

            if float(current_pos[2]) < climb_gate_z and xy_d > 8.0:
                # Stage 1: climb first, only slight drift toward goal.
                desired_z = climb_gate_z
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.42, 0.18, 0.42))
                xy_speed = 0.16

            elif xy_d > 18.0:
                # Stage 2: medium-fast high cruise.
                desired_z = platform_top_z_est + 9.5
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.28, -0.035, 0.22))
                xy_speed = 0.62

            elif xy_d > 7.0:
                # Stage 3: brake while staying safely above platform.
                desired_z = platform_top_z_est + 5.5
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.30, -0.055, 0.15))
                xy_speed = min(0.42, max(0.18, xy_d * 0.052))

            elif xy_d > 1.30:
                desired_z = platform_top_z_est + 1.65
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.38, -0.075, 0.10))
                xy_speed = min(0.20, max(0.055, xy_d * 0.10))

            elif xy_d > 0.62:
                # Final centering.
                desired_z = platform_top_z_est + 0.55
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.44, -0.075, 0.06))
                xy_speed = min(0.10, max(0.030, xy_d * 0.10))

            else:
                # Contact descent. Still below LANDING_MAX_VZ=0.5.
                self._adjusted_landing_ticks_v14 += 1

                desired_z = platform_top_z_est - 0.22
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.70, -0.11, 0.010))

                if self._adjusted_landing_ticks_v14 > 50:
                    vz_cmd = min(vz_cmd, -0.13)
                if self._adjusted_landing_ticks_v14 > 100:
                    vz_cmd = min(vz_cmd, -0.17)

                xy_speed = min(0.026, xy_d * 0.09)
