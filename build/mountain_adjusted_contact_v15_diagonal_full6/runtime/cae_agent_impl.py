...
class DroneFlightController:
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # MOUNTAIN_ADJUSTED_DIAGONAL_V15_DIAGNOSTIC
        if not hasattr(self, "_adjusted_goal_v15"):
            self._adjusted_goal_v15 = None
        if not hasattr(self, "_adjusted_landing_ticks_v15"):
            self._adjusted_landing_ticks_v15 = 0

        if self._adjusted_goal_v15 is None and self.step_count < 20:
            adjusted_start_v15 = np.array([-75.161464712166, 65.809547200952, 10.717666598692], dtype=np.float64)
            raw_task_start_v15 = np.array([-75.1614647121664, 65.80954720095166, -0.6810095881042477], dtype=np.float64)
            if (
                float(np.linalg.norm(current_pos - adjusted_start_v15)) < 4.0
                or float(np.linalg.norm(current_pos - raw_task_start_v15)) < 4.0
            ):
                self._adjusted_goal_v15 = np.array([-41.101316360082, 43.352505226496, 28.096424923198], dtype=np.float64)

        if self._adjusted_goal_v15 is not None:
            target_pos = np.asarray(self._adjusted_goal_v15, dtype=np.float64)
            self.target_vel = np.zeros(3, dtype=np.float64)
            dist_xy = float(np.linalg.norm(current_pos[:2] - target_pos[:2]))
        ...
        # MOUNTAIN_ADJUSTED_DIAGONAL_CONTACT_CONTROLLER_V15_DIAGNOSTIC
        if getattr(self, "_adjusted_goal_v15", None) is not None and not bool(cs.landing_phase):
            goal = np.asarray(self._adjusted_goal_v15, dtype=np.float64)
            ...
            if xy_d > 22.0:
                # Diagonal climb-cruise: cover distance while climbing.
                desired_z = platform_top_z_est + 6.5
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.55, 0.22, 0.46))
                xy_speed = 0.54

            elif xy_d > 10.0:
                # Continue closing, still above platform.
                desired_z = platform_top_z_est + 4.0
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.40, -0.06, 0.28))
                xy_speed = min(0.50, max(0.24, xy_d * 0.040))

            elif xy_d > 3.0:
                # Brake into platform vicinity.
                desired_z = platform_top_z_est + 1.80
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.38, -0.09, 0.14))
                xy_speed = min(0.26, max(0.08, xy_d * 0.075))

            elif xy_d > 0.75:
                # Center over pad.
                desired_z = platform_top_z_est + 0.55
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.42, -0.09, 0.07))
                xy_speed = min(0.11, max(0.035, xy_d * 0.095))

            else:
                # Contact descent.
                self._adjusted_landing_ticks_v15 += 1
                desired_z = platform_top_z_est - 0.24
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.72, -0.12, 0.010))

                if self._adjusted_landing_ticks_v15 > 45:
                    vz_cmd = min(vz_cmd, -0.14)
                if self._adjusted_landing_ticks_v15 > 90:
                    vz_cmd = min(vz_cmd, -0.18)

                xy_speed = min(0.024, xy_d * 0.080)
