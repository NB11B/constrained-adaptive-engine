...
class DroneFlightController:
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # MOUNTAIN_RAWFAR_OFFSET_SWEEP
        #
        # Corrected sweep: prior target offsets used raw_search_dist_xy < 14,
        # which mostly hit Village. Mountain appears in raw_search_dist_xy > 20.
        #
        # Do not force terrain_id. Only correct target column late in the raw-far regime.
        if (
            self.initial_dist_xy is not None
            ...
            target_pos = raw_search_target + np.array([8.000000, 0.000000, 2.000000], dtype=np.float64)
            target_pos[2] = max(float(target_pos[2]), 0.20 + 2.000000)
            self.target_vel = np.zeros(3, dtype=np.float64)
            dist_xy = float(np.linalg.norm(current_pos[:2] - target_pos[:2]))
