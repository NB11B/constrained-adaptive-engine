...
class DroneFlightController:
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # Diagnostic gate variants.
        # v2/v3 produced exact baseline, so we need to know whether the strict gate
        # is false, or whether action modifications are being overwritten later.
        mountain_geom_strict = (
            terrain_id == 3
            and self.initial_dist_xy is not None
            and float(self.initial_dist_xy) > 30.0
        )
        ...
        # Keep old variable name available for any later injected code.
        mountain_geom = mountain_geom_strict
        ...
        # Mountain diagnostic probe.
        # Sparse prints only. Do not use this as a submission candidate.
        dbg_tick = int(getattr(self, "_mountain_probe_tick", 0)) + 1
        self._mountain_probe_tick = dbg_tick

        if dbg_tick <= 20 or dbg_tick % 25 == 0 or mountain_geom_loose:
            try:
                print(
                    "[MOUNTAIN_PROBE]",
                    "tick=", dbg_tick,
                    "terrain_id=", int(terrain_id),
                    "strict=", bool(mountain_geom_strict),
                    "loose=", bool(mountain_geom_loose),
                    "initial_dist_xy=", None if self.initial_dist_xy is None else round(float(self.initial_dist_xy), 3),
                    "dist_xy=", round(float(dist_xy), 3),
                    "spawn_z=", round(float(self.spawn_z), 3),
                    "z=", round(float(current_pos[2]), 3),
                    "agl=", round(float(agl), 3),
                    "target_z=", round(float(target_pos[2]), 3),
                    "landing=", bool(cs.landing_phase),
                    "action_before=", [round(float(x), 3) for x in action],
                    flush=True,
                )
            except Exception as e:
                print("[MOUNTAIN_PROBE_ERROR]", repr(e), flush=True)

        # Also test whether action modification survives downstream logic.
        # If strict gate is true, force an obvious marker action before city limiter.
        # This is diagnostic only.
        if mountain_geom_strict and not bool(cs.landing_phase) and dist_xy > 12.0:
            action[2] = 0.777
            action[3] = max(float(action[3]), 0.777)
            if dbg_tick <= 20 or dbg_tick % 25 == 0:
                print(
                    "[MOUNTAIN_PROBE_MARKER_APPLIED]",
                    "tick=", dbg_tick,
                    "action_marker=", [round(float(x), 3) for x in action],
                    flush=True,
                )
