...
class DroneFlightController:
    ...
    def _estimate_target(self, current_pos: np.ndarray, search_vec: np.ndarray) -> np.ndarray:
        ...
        # MOUNTAIN_NEGATIVE_TERRAIN_Z_FIX_V7B
        #
        # Type 3 Mountain goals are generated at terrain surface z, which can be
        # negative. Do not clamp Mountain-like terrain-surface goals to +0.10.
        spawn_z_v = float(self.spawn_z) if self.spawn_z is not None else float(current_pos[2])
        raw_target_z_v = float(raw_target[2])
        allow_negative_terrain_goal = (
            raw_target_z_v < -0.05
            or spawn_z_v < 0.0
            or float(current_pos[2]) < -0.05
        )

        if allow_negative_terrain_goal:
            raw_target[2] = raw_target_z_v
        elif abs(float(search_vec[2])) < 0.25:
            raw_target[2] = 0.20
        else:
            raw_target[2] = max(0.10, raw_target_z_v)
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # STANDARDIZED_GOAL_CORRECTIONS_V7B
