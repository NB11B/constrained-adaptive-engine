...
class DroneFlightController:
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # BEHAVIORAL PROBE ONLY.
        # This intentionally forces a visible action marker when the gate is true.
        # If benchmark scores do not change, this code path/gate is not reaching final control.
        if (self.initial_dist_xy is not None and float(self.initial_dist_xy) > 20.0 and dist_xy > 8.0) and not bool(cs.landing_phase):
            if dist_xy > 2.0:
                action[0] = 0.0
                action[1] = 0.0
                action[2] = 0.95
