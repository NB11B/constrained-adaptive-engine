...
class DroneFlightController:
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # Strict Mountain-only geometry gate.
        # v1 failed because terrain_id == 3 alone contaminated Open/Forest.
        # v2 proved this stricter gate preserves baseline behavior.
        mountain_geom = (
            terrain_id == 3
        ...
        # Mountain v3 strict anti-dive assist.
        #
        # Root cause hypothesis:
        #   Mountain target z often collapses near ground, so target_alt_assist can be too low.
        #   If the drone starts above that computed target, the controller commands descent
        #   before it has acquired the platform.
        #
        # This patch does NOT use terrain_id alone. It only activates under the same strict
        # mountain_geom gate that v2 proved was non-destructive across Open/Forest/Warehouse.
        #
        # Behavior:
        #   - Far from target: prohibit descent and keep forward/turn energy.
        #   - Mid-range: still avoid aggressive descent.
        #   - Close range / landing phase: leave landing behavior alone.
        if mountain_geom and not bool(cs.landing_phase):
            if dist_xy > 12.0:
                # Long approach: do not allow descent. Hold or climb mildly.
                if action[2] < 0.10:
                    action[2] = 0.10
                action[3] = max(float(action[3]), 0.30)

            elif dist_xy > 4.0:
                # Mid approach: allow less intervention, but still prevent diving.
                if action[2] < 0.03:
                    action[2] = 0.03
                action[3] = max(float(action[3]), 0.24)

            else:
                # Close approach: do not interfere with final controller except to
                # prevent a hard premature dive before official landing phase.
                if action[2] < -0.08:
                    action[2] = -0.08
