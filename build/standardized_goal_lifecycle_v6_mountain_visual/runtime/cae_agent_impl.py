...
class DroneFlightController:
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # STANDARDIZED_GOAL_LIFECYCLE_V6
        #
        # Same scenario-section format for every map:
        #   name
        #   lock
        #   init window
        #   raw_search window
        ...
        #   terminal settings
        #
        # Mountain differs only in parameters/source strategy, not format.
        ...
        active_goal_scenario = None
        active_goal_cfg = None

        if not hasattr(self, "_far_start_lock"):
            self._far_start_lock = None
        if not hasattr(self, "_far_seen_raw_le20"):
            self._far_seen_raw_le20 = False
        if not hasattr(self, "_mountain_visual_ticks"):
            self._mountain_visual_ticks = 0
        if not hasattr(self, "_mountain_terminal_ticks"):
            self._mountain_terminal_ticks = 0

        is_far_start = (
            self.initial_dist_xy is not None
            and 40.0 < float(self.initial_dist_xy) <= 50.0
        )

        # Far-start resolver:
        # Village proves itself by entering raw_search_dist_xy <= 20.
        # Mountain is the far-start case that stays raw-far long enough.
        if is_far_start and float(raw_search_dist_xy) <= 20.0:
            self._far_seen_raw_le20 = True
            self._far_start_lock = "village"

        if (
            is_far_start
            and self._far_start_lock is None
            and not bool(self._far_seen_raw_le20)
            and int(self.step_count) > 900
            and float(raw_search_dist_xy) > 20.0
        ):
            self._far_start_lock = "mountain"
        ...
        scenario_rows = [
            {
                ...
                "offset": np.array([-0.25, -1.42, -1.02], dtype=np.float64),
                "z_floor": None,
                "terminal_enabled": False,
                "terminal_gate_xy": 0.70,
                "hold_z_offset": 0.015,
                "xy_speed": 0.10,
                "vz_min": -0.020,
                "vz_max": 0.180,
            },
            {
                ...
                "step_min": 250,
                "target_source": "fixed_offset",
                "offset": np.array([1.73, -6.25, 0.20], dtype=np.float64),
                "z_floor": 0.20,
                "terminal_enabled": False,
                "terminal_gate_xy": 0.70,
                "hold_z_offset": 0.015,
            ...
            },
            {
                ...
                "step_min": 250,
                "target_source": "fixed_offset",
                "offset": np.array([6.51, -3.53, 1.63], dtype=np.float64),
                "z_floor": 1.83,
                "terminal_enabled": False,
                "terminal_gate_xy": 0.85,
                "hold_z_offset": 0.060,
                "xy_speed": 0.11,
                "vz_min": -0.035,
                "vz_max": 0.160,
            ...
            {
                # Explicit Village raw-far boost.
                #
                # This preserves the v1-style boost as a first-class Village
                # phase instead of letting Mountain accidentally alter Village.
                "name": "village_rawfar_boost",
                "lock": None,
                "init_min": 40.0,
                "init_max": 50.0,
                "raw_min": 20.0,
                ...
                "step_min": 250,
                "target_source": "fixed_offset",
                "offset": np.array([0.0, 0.0, 2.0], dtype=np.float64),
                "z_floor": 2.20,
                "terminal_enabled": False,
                "terminal_gate_xy": 0.95,
                "hold_z_offset": 0.035,
                "xy_speed": 0.12,
                "vz_min": -0.030,
                "vz_max": 0.160,
            ...
            {
                # Mountain full lifecycle.
                #
                # The key missing piece is target_source. Unlike Open/Forest,
                # Mountain does not have a known GOAL-search offset. Use raw
                # search target as the base, but later add a visual platform
                # bearing assist in the Mountain-local controller below.
                "name": "mountain",
                "lock": "mountain",
                "init_min": 40.0,
                "init_max": 50.0,
                ...
                "raw_max": 999.0,
                "step_min": 900,
                "target_source": "visual_assisted_raw",
                "offset": np.array([0.0, 0.0, 0.0], dtype=np.float64),
                "z_floor": 1.20,
                "terminal_enabled": True,
                "terminal_gate_xy": 2.20,
                "hold_z_offset": 0.12,
                "xy_speed": 0.16,
                "vz_min": -0.060,
                "vz_max": 0.160,
        ...
        if self.initial_dist_xy is not None:
            for corr in scenario_rows:
                lock_name = corr.get("lock", None)
                if lock_name is not None and self._far_start_lock != lock_name:
                    continue

                # Do not let explicit raw-far Village boost run after the map
                # has locked as Mountain.
                if corr["name"] == "village_rawfar_boost" and self._far_start_lock == "mountain":
                    continue

                if (
                    float(corr["init_min"]) < float(self.initial_dist_xy) <= float(corr["init_max"])
                    and float(corr["raw_min"]) < float(raw_search_dist_xy) <= float(corr["raw_max"])
                    and int(self.step_count) > int(corr["step_min"])
                ):
                    target_pos = raw_search_target + corr["offset"]
                    if corr["z_floor"] is not None:
                        target_pos[2] = max(float(target_pos[2]), float(corr["z_floor"]))
                    self.target_vel = np.zeros(3, dtype=np.float64)
                    dist_xy = float(np.linalg.norm(current_pos[:2] - target_pos[:2]))
                    active_goal_scenario = str(corr["name"])
                    active_goal_cfg = corr
                    break
        ...
        # MOUNTAIN_VISUAL_LIFECYCLE_V6
        #
        # Mountain-only controller extension:
        #   - runs only after far-start lock resolves to Mountain
        #   - uses same scenario cfg fields
        #   - adds visual platform bearing assist from existing _detect_platform_bearing()
        #   - does not affect known-good City/Open/Forest/Village/Warehouse terminal paths
        if (
            active_goal_cfg is not None
            and active_goal_scenario == "mountain"
            and getattr(self, "_far_start_lock", None) == "mountain"
            and not bool(cs.landing_phase)
        ):
            bearing = self._detect_platform_bearing(depth)
            xy_err = target_pos[:2] - current_pos[:2]
            xy_d = float(np.linalg.norm(xy_err))

            # Default approach: move toward standardized Mountain target.
            if xy_d > 1e-6:
                xy_dir = xy_err / xy_d
            else:
                xy_dir = np.zeros(2, dtype=np.float64)

            # Visual assist: if a compact platform-like blob is visible,
            # yaw toward it and add a small lateral component. This supplies
            # the missing Mountain target-source term without hardcoding GOAL.
            yaw_assist = yaw_norm
            lateral = np.zeros(2, dtype=np.float64)
            if bearing is not None:
                self._mountain_visual_ticks += 1
                b = float(np.clip(bearing, -0.65, 0.65))
                yaw_assist = float(np.clip(yaw_norm + 0.55 * b, -1.0, 1.0))
                # Perpendicular nudge in body-agnostic world XY approximation.
                # This is intentionally small so it cannot dominate the target vector.
                lateral = np.array([-xy_dir[1], xy_dir[0]], dtype=np.float64) * float(np.clip(b, -0.35, 0.35))
            else:
                self._mountain_visual_ticks = max(0, int(self._mountain_visual_ticks) - 1)

            # Approach altitude: stay above terrain while closing XY.
            # Terminal altitude: once close, hold just above target z.
            if dist_xy > float(active_goal_cfg["terminal_gate_xy"]):
                desired_z = max(float(target_pos[2]) + 3.2, float(current_pos[2]) - 0.04)
                z_err = desired_z - float(current_pos[2])
                vz_cmd = float(np.clip(z_err * 0.45, -0.050, 0.140))
                xy_speed = min(0.22, max(0.08, dist_xy * 0.10))
            else:
                self._mountain_terminal_ticks += 1
                hold_z = float(target_pos[2]) + float(active_goal_cfg["hold_z_offset"])
                z_err = hold_z - float(current_pos[2])
                vz_cmd = float(np.clip(
                    z_err * 0.85,
                    float(active_goal_cfg["vz_min"]),
                    float(active_goal_cfg["vz_max"]),
                ))
                xy_speed = min(float(active_goal_cfg["xy_speed"]), max(0.04, dist_xy * 0.45))

            xy_cmd = xy_dir * xy_speed + lateral * 0.035
            action = _action_from_velocity(
                float(xy_cmd[0]),
                float(xy_cmd[1]),
                vz_cmd,
                float(np.linalg.norm([xy_cmd[0], xy_cmd[1], vz_cmd])),
                yaw_assist,
            )
        ...
        self._trace_step({
            ...
            "goal_scenario": active_goal_scenario,
            "far_lock": getattr(self, "_far_start_lock", None),
            "far_seen_le20": bool(getattr(self, "_far_seen_raw_le20", False)),
            "raw_search_dist_xy": round(float(raw_search_dist_xy), 4),
            "mtn_visual_ticks": int(getattr(self, "_mountain_visual_ticks", 0)),
            "mtn_terminal_ticks": int(getattr(self, "_mountain_terminal_ticks", 0)),
