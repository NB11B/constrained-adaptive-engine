...
class DroneFlightController:
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # STANDARDIZED_GOAL_LIFECYCLE_V3
        #
        # Every scenario uses the same correction format:
        #   name
        #   initial_dist_xy window
        #   raw_search_dist_xy window
        #   step_min
        #   offset_xyz
        #   z_floor
        #   terminal hold parameters
        #
        # Important architectural change:
        #   Mountain is no longer only a terrain_id==3 safety/descent exception.
        #   It has a complete target-correction + terminal-hold lifecycle.
        ...
        goal_corrections = [
            {
                ...
                "terminal_hold_z_offset": 0.015,
                "terminal_vz_min": -0.020,
                "terminal_vz_max": 0.180,
                "terminal_xy_speed": 0.10,
            },
            {
                "name": "open",
                "init_min": 14.0,
            ...
            },
            {
                ...
                "terminal_hold_z_offset": 0.060,
                "terminal_vz_min": -0.035,
                "terminal_vz_max": 0.160,
                "terminal_xy_speed": 0.11,
            },
            {
                # Mountain is now a full lifecycle block.
                #
                # Current best information:
                #   - Mountain/Village far-start regime: initial_dist_xy 40-50.
                #   - Village improvement appeared from the standardized row behavior.
                #   - Mountain remains floor score, so v3 gives Mountain a terminal
                #     hold/descent lifecycle instead of terrain_id==3-only exceptions.
                #
                # Keep XY offset neutral in v3 because coarse XY sweeps did not lift
                # Mountain off floor. Focus on holding the corrected raw target column
                # and entering a stable terminal path.
                "name": "mountain",
                "init_min": 40.0,
                "init_max": 50.0,
                "raw_min": 20.0,
                ...
                "z_floor": 1.20,
                "terminal_gate_xy": 1.65,
                "terminal_hold_z_offset": 0.12,
                "terminal_vz_min": -0.060,
                "terminal_vz_max": 0.160,
                "terminal_xy_speed": 0.16,
            },
            {
                # Village uses the same format. This preserves the standardized v1
                # behavior that improved Village, while keeping it explicit.
                "name": "village",
                "init_min": 40.0,
                "init_max": 50.0,
                "raw_min": 10.0,
                ...
                "terminal_hold_z_offset": 0.035,
                "terminal_vz_min": -0.030,
                "terminal_vz_max": 0.160,
                "terminal_xy_speed": 0.12,
            },
        ]

        if self.initial_dist_xy is not None:
            for corr in goal_corrections:
                if (
        ...
        if (
            ...
            and active_goal_scenario is None
        ):
        ...
        # STANDARDIZED_TERMINAL_HOLD_V3
        #
        # Same terminal-hold format for all corrected scenarios.
        # This generalizes the previous Open-only terminal z guard.
        if (
            active_terminal_cfg is not None
            and dist_xy < float(active_terminal_cfg["terminal_gate_xy"])
            and current_pos[2] < target_pos[2] + 0.95
        ):
            hold_z = float(target_pos[2]) + float(active_terminal_cfg["terminal_hold_z_offset"])
            z_err = hold_z - float(current_pos[2])
            vz_hold = float(np.clip(
                z_err * 0.95,
                float(active_terminal_cfg["terminal_vz_min"]),
                float(active_terminal_cfg["terminal_vz_max"]),
            ))

            xy_err = target_pos[:2] - current_pos[:2]
            xy_d = float(np.linalg.norm(xy_err))
            if xy_d > 1e-6:
                xy_speed = min(float(active_terminal_cfg["terminal_xy_speed"]), xy_d * 0.50)
                xy_v = xy_err / xy_d * xy_speed
            else:
        ...
        legacy_mountain_enabled = active_goal_scenario != "mountain"

        mountain_low_agl_guard = (
            legacy_mountain_enabled
            and terrain_id == 3
        ...
        if dist_xy < term_gate_xy and agl < TERMINAL_ASSIST_AGL_M and is_near_ground_level:
            ...
            elif legacy_mountain_enabled and terrain_id == 3 and dist_xy > 5.0:
