...
class DroneFlightController:
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # STANDARDIZED_GOAL_CORRECTIONS_V5_LOCKED
        #
        # Every scenario row has the same format.
        # Mountain/Village are resolved by a far-start lock so Mountain terminal
        # behavior does not fire on Village during the shared raw-far phase.
        ...
        # Far-start resolver state. Use dynamic attributes so __init__ does not
        # need to be patched.
        ...
        # Do not declare Mountain immediately. Village begins in the same raw-far
        # space. Only lock Mountain after giving Village time to prove itself by
        # passing through raw_search_dist_xy <= 20.
        if (
            is_far_start
        ...
        # MOUNTAIN_LOCKED_TERMINAL_LIFECYCLE_V5
        #
        # Mountain-only terminal assist after far-start resolver locks Mountain.
        # Does not affect City/Open/Forest/Village.
        if (
            active_goal_cfg is not None
            and active_goal_scenario == "mountain"
            and bool(active_goal_cfg.get("terminal_enabled", False))
            and getattr(self, "_far_start_lock", None) == "mountain"
            and dist_xy < float(active_goal_cfg["terminal_gate_xy"])
            and current_pos[2] < target_pos[2] + 1.25
        ):
            hold_z = float(target_pos[2]) + float(active_goal_cfg["hold_z_offset"])
            z_err = hold_z - float(current_pos[2])
            vz_hold = float(np.clip(z_err * 0.85, -0.055, 0.155))

            xy_err = target_pos[:2] - current_pos[:2]
            xy_d = float(np.linalg.norm(xy_err))
            if xy_d > 1e-6:
                xy_v = xy_err / xy_d * min(0.13, xy_d * 0.45)
            else:
