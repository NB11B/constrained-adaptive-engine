...
class DroneFlightController:
    ...
    def _kg_file_trace(self, payload: dict) -> None:
        """Best-effort runtime trace sink for benchmark containers.

        We cannot assume stdout/stderr is surfaced by swarm.benchmark.
        Try several likely writable locations. The script will search afterward.
        """
        try:
            line = json.dumps(payload, separators=(",", ":")) + "\n"
            paths = [
                os.environ.get("CAE_KG_TRACE_FILE"),
                "/tmp/cae_goal_trace.jsonl",
                "/tmp/runtime_goal_corr_trace.jsonl",
                "./cae_goal_trace.jsonl",
                os.path.join(os.getcwd(), "cae_goal_trace.jsonl"),
            ]
            for path in paths:
                if not path:
                    continue
                try:
                    ...
                    with open(path, "a", encoding="utf-8") as f:
                        f.write(line)
                    # Once one write succeeds, continue trying /tmp as well only
                    # if this was not already /tmp. This maximizes chance of harvest.
                except Exception:
                    pass
        except Exception:
            pass
    ...
    def act(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # RUNTIME_GOAL_CORR_FILE_TRACE_V3
        if (
            applied_goal_correction is not None
            or self.step_count < 8
            or self.step_count % 100 == 0
        ):
            self._kg_file_trace(
                {
                    "phase": "pre_terrain",
